...
export function EventsAdmin({
  ...
  const upcoming = events.filter((event) => new Date(event.event_date).getTime() >= now);
  const past = events.filter((event) => new Date(event.event_date).getTime() < now);
        ...
        <div className="flex flex-col gap-12">
          {upcoming.length > 0 && (
            <div>
              ...
              <EventTable events={upcoming} venues={venues} profiles={profiles} />
            </div>
          )}
          {past.length > 0 && (
            <div>
              <h3 className={GROUP_HEADING}>{t("PastHeading")}</h3>
              <EventTable events={past} venues={venues} profiles={profiles} />
            </div>
          )}
        </div>
      )}
