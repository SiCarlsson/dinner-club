...
* **Admin:** Exclusive access to a dedicated admin dashboard with capabilities to generate invitation links, create/edit/delete dinners, and curate restaurant profile data.
* **Member:** Restricted access allowing users to view the dinner calendar, submit RSVPs, manage their personal profile, and rate visited locations.

---

## 📅 2. Calendar & Dinner Management

### 2.1 Dinner Calendar
A centralized dashboard providing an overview of upcoming dinners, detailing essential parameters such as dates, times, and restaurant profiles.
...
Interactive capabilities enabling members to explicitly declare their attendance status ("Attending" or "Cannot Attend") for scheduled dinners.
...
Persistent profile fields allowing members to save long-term allergies or dietary restrictions. Additionally, the RSVP flow must include an optional free-text field for temporary, dinner-specific food comments. Admins have access to a consolidated view compiling all participant requirements ahead of table bookings.
...
A notification layer triggered by key dinners (RSVP deadlines, secret location reveals, and post-dinner rating requests). While initial communication relies on email (via Supabase/SMTP), the underlying Node.js architecture must be modularized using webhooks or abstract API structures to seamlessly support future integrations with external chat clients like Discord, Telegram, or WhatsApp.
...
### 4.1 Eligibility
Stricter backend validation ensures that only members who held an active "Attending" status for a specific dinner are permitted to write records to the rating database.
